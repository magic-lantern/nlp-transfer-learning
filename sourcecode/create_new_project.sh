...
echo "   2) Check out the new repository and change into the base directory of the repository"
echo "   3) Create the first wiki page in your repository manually."
...
find . -type f ! -path '*/\.git/*' -exec gsed -i 's/callahantiff/{$USER_NAME}/g' {} \;
find . -type f ! -path '*/\.git/*' -exec gsed -i 's/Abra-Collaboratory/{$PROJECT_NAME}/g' {} \;
